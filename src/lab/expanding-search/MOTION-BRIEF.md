...
**Keyboard:** ArrowDown from the input enters the list; ArrowUp/ArrowDown, Home and End move within it with a roving tabindex so Tab leaves the list; ArrowUp on the first row returns to the input; Escape anywhere collapses the card and returns focus to the input with the whole term selected, so it can be typed over or kept. Choosing a result (click, or Enter on a focused row) ends the search instead: the card collapses and the field clears and takes focus again. The highlight follows focus.
